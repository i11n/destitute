...
export module AssertionRouter {
  ...
  function assertRouteExists<T extends IConstructor<any>>(Class: T | string) {
    ROUTES.forEach(route => {
      ...
      if (routeClass === Class) {
        throw new Error(`A route matching "${typeof Class === "string" ? Class : Class.name}" is already mapped to "${assertion.name}". Use "setRoute()" to override.`);
      }
  ...
  export function addRoute<T extends IConstructor<any>>(Class: T | string, assertion: IConstructor<IAssertion>) {
    assertRouteExists(Class);

    ROUTES.push([Class, assertion]);
  }

  export function setRoute<T extends IConstructor<any>>(Class: T | string, assertion: IConstructor<IAssertion>) {
    ...
    if (!foundRoute) {
      throw new Error(`No route matching "${typeof Class === "string" ? Class : Class.name}" was found. Use "addRoute()" or "spliceRoute()" instead.`);
    }
  }
  ...
  export function matchRoute(value: any): IConstructor<IAssertion> {
    ...
    if (typeof value === "object" || typeof value === "function") {
      valueType = !value ? "base" : (value as Object).constructor as IConstructor<any>;
    }
    ...
    throw new Error(`No route matching "${typeof valueType === "string" ? valueType : valueType.name}" for value (${value}) was found.`);
  }
